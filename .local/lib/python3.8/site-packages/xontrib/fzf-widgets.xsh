...
def fzf_insert_history(event):
    ...
    proc = subprocess.Popen(popen_args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, universal_newlines=True, env=env)
...
def fzf_insert_file(event, dirs_only=False):
    ...
    if dirs_only:
        ...
        # env['FZF_DEFAULT_OPTS'] += ' --preview "bat --style=numbers --color=always --line-range=:200 {}"'
        env['FZF_DEFAULT_OPTS'] += ' --preview "fzf_prev {}"'
    # fzf call
    choice = subprocess.run([get_fzf_binary_path(), '-m', '--reverse'], stdout=subprocess.PIPE, universal_newlines=True, env=env).stdout.strip()
    if cwd:
        os.chdir(cwd)
...
def fzf_prompt_from_string(string):
    ...
    if 'FZF_DEFAULT_OPTS' in ${...}:
        env['FZF_DEFAULT_OPTS'] = $FZF_DEFAULT_OPTS
    choice = subprocess.run([get_fzf_binary_path(), '--tiebreak=index', '+m', '--reverse', '--height=40%'], input=string, stdout=subprocess.PIPE, universal_newlines=True, env=env).stdout.strip()
    return choice
